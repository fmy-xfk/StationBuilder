...
public class RailBuilderConfig {
    ...
    public ResourceLocation railType = StationBuilder.isMtrLoaded() ?
    ...
    public ResourceLocation catenaryBlock = StationBuilder.isMsdLoaded() ?
            ResourceLocation.fromNamespaceAndPath("msd", "catenary_connector") :
            ResourceLocation.fromNamespaceAndPath("minecraft", "cobweb");
    public ResourceLocation catenaryBridgePillar = StationBuilder.isMsdLoaded() ?
    ...
    public ResourceLocation catenaryTunnelPillar = StationBuilder.isMsdLoaded() ?
    ...
    public static RailBuilderConfig fromItem(ItemStack stack) {
        ...
        CompoundTag root = stack.getOrDefault(ModComponents.RAIL_BUILDER_DATA.get(), CustomData.EMPTY).copyTag();
        if (root.contains("railBuilderConfig", Tag.TAG_COMPOUND)) {
    ...
    public void fromNbt(CompoundTag nbt) {
        ...
        if (nbt.contains("railType", Tag.TAG_STRING))
            railType = ResourceLocation.parse(nbt.getString("railType"));
        ...
        if (nbt.contains("ballastBlock", Tag.TAG_STRING))
            ballastBlock = ResourceLocation.parse(nbt.getString("ballastBlock"));
        ...
        if (nbt.contains("bridgeGuardRailBlock", Tag.TAG_STRING))
            bridgeGuardRailBlock = ResourceLocation.parse(nbt.getString("bridgeGuardRailBlock"));
        if (nbt.contains("bridgeBlock", Tag.TAG_STRING))
            bridgeBlock = ResourceLocation.parse(nbt.getString("bridgeBlock"));
        if (nbt.contains("bridgePillarBlock", Tag.TAG_STRING))
            bridgePillarBlock = ResourceLocation.parse(nbt.getString("bridgePillarBlock"));
        ...
        if (nbt.contains("tunnelWallBlock", Tag.TAG_STRING))
            tunnelWallBlock = ResourceLocation.parse(nbt.getString("tunnelWallBlock"));
        if (nbt.contains("tunnelCeilingBlock", Tag.TAG_STRING))
            tunnelCeilingBlock = ResourceLocation.parse(nbt.getString("tunnelCeilingBlock"));
        if (nbt.contains("tunnelFloorBlock", Tag.TAG_STRING))
            tunnelFloorBlock = ResourceLocation.parse(nbt.getString("tunnelFloorBlock"));
        ...
        if (nbt.contains("useCatenary", Tag.TAG_BYTE))
            useCatenary = nbt.getBoolean("useCatenary");
        if (nbt.contains("isVanillaCatenary", Tag.TAG_BYTE))
        ...
        if (nbt.contains("catenaryBlock", Tag.TAG_STRING))
            catenaryBlock = ResourceLocation.parse(nbt.getString("catenaryBlock"));
        if (nbt.contains("catenaryBridgePillar", Tag.TAG_STRING))
            catenaryBridgePillar = ResourceLocation.parse(nbt.getString("catenaryBridgePillar"));
        if (nbt.contains("catenaryTunnelPillar", Tag.TAG_STRING))
            catenaryTunnelPillar = ResourceLocation.parse(nbt.getString("catenaryTunnelPillar"));
    }
}
